...
def graph_to_kml(graph, filename):
    ...
    # 2) Añadir cada nodo como Placemark→Point
    ...
    # 3) Añadir cada segmento como Placemark→LineString
...
def main():
    ...
    NAV = os.path.join(base, "Cat_nav.txt")
    SEG = os.path.join(base, "Cat_seg.txt")
    AER = os.path.join(base, "Cat_aer.txt")

    # Carga del espacio aéreo
    aspace = AirSpace()
    ...
    # Exportar a KML
    out = os.path.join(base, "espacio_cataluna.kml")
    graph_to_kml(g, out)
